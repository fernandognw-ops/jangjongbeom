    ...
    <div className="grid min-w-0 grid-cols-2 gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 md:gap-4">
      {ITEMS.map((item) => {
        ...
        return (
          <div
            key={item.id}
            className={`flex min-h-[80px] flex-col overflow-hidden rounded-xl border p-2 shadow-card transition-colors md:min-h-[120px] md:p-5 ${
              isShort
                ? "border-amber-200 bg-amber-50"
                : "border-slate-200 bg-white"
            }`}
          >
            <div className="mb-0.5 shrink-0 text-[10px] font-medium uppercase tracking-wider text-slate-600 md:mb-2 md:text-xs">
              {item.name}
            </div>
            <div
              className={`min-w-0 overflow-hidden text-ellipsis tabular-nums font-bold text-slate-800 ${
            ...
            <div
              className={`mt-0.5 min-w-0 overflow-hidden text-ellipsis text-slate-600 md:mt-2 md:text-sm ${
            ...
            {isShort && safetyStock > 0 && (
              <div className="mt-0.5 shrink-0 text-[10px] font-medium text-amber-700 md:mt-2 md:text-xs">
