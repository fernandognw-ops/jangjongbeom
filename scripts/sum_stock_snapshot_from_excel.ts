 ...
 *
 * DB 비교: dest_warehouse(판매채널) 기준
 *   SELECT dest_warehouse, SUM(quantity) FROM inventory_stock_snapshot GROUP BY 1;
 */
...
async function main() {
  ...
  console.log(JSON.stringify({
    ...
    note: "엑셀 「판매 채널」→ normalizeSalesChannelKr (DB dest_warehouse·quick API와 동일)",
  }, null, 2));
}
